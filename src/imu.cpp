...
Eigen::Matrix3d euler2Rotation(Eigen::Vector3d eulerAngles) {
  ...
  Eigen::Matrix3d RIb;
  RIb << cy * cp, cy * sp * sr - sy * cr, sy * sr + cy * cr * sp, sy * cp,
      cy * cr + sy * sr * sp, sp * sy * cr - cy * sr, -sp, cp * sr, cp * cr;
  return RIb;
}
...
MotionData IMU::MotionModel(double t) {
  ...
  float K = M_PI / 10;  // 20 * K = 2pi 　　由于我们采取的是时间是20s,
                        // 系数K控制yaw正好旋转一圈，运动一周
  ...
  //    Eigen::Vector3d eulerAngles(0.0,0.0, K*t );   // roll ~ 0, pitch ~ 0,
...
void IMU::testImu(std::string src, std::string dist) {
  ...
  for (int i = 1; i < imudata.size(); ++i) {
    ...
    Eigen::Vector3d acc_w =
        (Qwb * dq * imupose.imu_acc + Qwb * imudata[i - 1].imu_acc -
