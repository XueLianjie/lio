...
// Static member variables in LIDARState class.
// Isometry3d LIDARState::T_cam0_cam1 = Isometry3d::Identity();
...
map<int, double> Lio::chi_squared_test_table;

Lio::Lio(ros::NodeHandle& pnh) : is_gravity_set(false),
                                 is_first_point_cloud(true),
                                 nh(pnh) {
  return;
}

bool Lio::loadParameters() {
  // Frame id
  ...
  state_server.imu_state.R_imu_livox = T_cam0_imu.linear().transpose();
  state_server.imu_state.t_livox_imu = T_cam0_imu.translation();
...
void Lio::imuCallback(
    const sensor_msgs::ImuConstPtr& msg) {
  ROS_INFO("imu time : %f ", msg->header.stamp.toSec());
...
void Lio::initializeGravityAndBias() {
...
void Lio::featureCallback(
    const CameraMeasurementConstPtr& msg) {
  ROS_INFO("feature time : %f ", msg->header.stamp.toSec());
...
void Lio::batchImuProcessing(const double& time_bound) {
...
void Lio::processModel(const double& time,
...
void Lio::predictNewState(const double& dt,
                          const Vector3d& gyro,
                          const Vector3d& acc) {
...
void Lio::stateAugmentation(const double& time) {
  ...
  state_server.cam_states[state_server.imu_state.id] =
      LIDARState(state_server.imu_state.id);
  LIDARState& cam_state = state_server.cam_states[state_server.imu_state.id];
...
void Lio::addFeatureObservations(  // stereo camera so u0 v0, u1 v1 4 datas
    const CameraMeasurementConstPtr& msg) {
...
void Lio::measurementJacobian(
    const StateIDType& cam_state_id,
    const FeatureIDType& feature_id,
    Matrix<double, 4, 6>& H_x, Matrix<double, 4, 3>& H_f, Vector4d& r) {
  // Prepare all the required data.
  const LIDARState& cam_state = state_server.cam_states[cam_state_id];
  ...
  // Cam1 pose.
  Matrix3d R_c0_c1 = LIDARState::T_cam0_cam1.linear();
  Matrix3d R_w_c1 = LIDARState::T_cam0_cam1.linear() * R_w_c0;
  Vector3d t_c1_w = t_c0_w - R_w_c1.transpose() * LIDARState::T_cam0_cam1.translation();
...
void Lio::findRedundantLIDARStates(
...
void Lio::pruneLIDARStateBuffer() {
  if (state_server.cam_states.size() < max_cam_state_size)
    return;

  // Find two camera states to be removed.
  vector<StateIDType> rm_cam_state_ids(0);
  findRedundantLIDARStates(rm_cam_state_ids);
...
void Lio::publish(const ros::Time& time) {
