...
#ifndef LIDAR_STATE_H
#define LIDAR_STATE_H
...
struct LIDARState {
  ...
  LIDARState(const StateIDType& new_id ): id(new_id), time(0),
    orientation(Eigen::Vector4d(0, 0, 0, 1)),
    position(Eigen::Vector3d::Zero()),
    orientation_null(Eigen::Vector4d(0, 0, 0, 1)),
...
typedef std::map<StateIDType, LIDARState, std::less<int>,
        Eigen::aligned_allocator<
        std::pair<const StateIDType, LIDARState> > > CamStateServer;
} // namespace msckf_vio
